...
var Notifications = {
  ...
  async markRead(nid) {
    try { await DB.collection('notifications').doc(nid).update({ read: true }); } catch(e) {}
  },

  async markAllRead(sp) {
    try {
      var r = await DB.collection('notifications').where({ salesperson_name: sp, read: false }).get();
      var list = r.data || [];
