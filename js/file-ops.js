/**
 * CloudBase 版 - 文件操作（纯云开发存储，无 Supabase 回退）
 */
const FileOps = {
  _openOrderId: null,
  ...
  async _load(orderId, userRole) {
    ...
    const order = res.data && res.data.length > 0 ? res.data[0] : null;
    ...
    if (!files.length) { html += '<p class="file-empty">暂无文件</p>'; }
      ...
      // 获取 CloudBase 临时下载链接
      const cbIds = files.filter(f => String(f.id).startsWith('cloud://')).map(f => f.id);
      let cbUrls = {};
      if (cbIds.length) {
        try {
          const r = await TCB.getTempFileURL({ fileList: cbIds });
          (r.fileList||[]).forEach((item, i) => {
            if (item.tempFileURL) cbUrls[cbIds[i]] = item.tempFileURL;
          });
        } catch(e) { console.warn('获取下载链接失败:', e); }
      }

      files.forEach(f => {
        const t = f.type || this._type(f.name||'');
        const dl = cbUrls[f.id] || '';
  ...
  async _upload(fileList) {
    ...
    for (const file of fileList) {
      try {
        ...
        const cloudPath = `${this._openOrderId}_${ts}_${safeName}`;
        const upRes = await TCB.uploadFile({ cloudPath, filePath: file });
        const meta = {
          id: upRes.fileID,
          name: file.name,
          size: file.size,
          type: this._type(file.name),
          uploader: Auth.getUser()?.full_name || Auth.getUser()?.email || '',
          uploaded_at: new Date().toISOString()
        };
        await API.attachFileToOrder(this._openOrderId, meta);
      } catch(e) {
  ...
  _replace(oldId) {
    ...
    i.onchange = async () => { if (i.files.length) FileOps._replaceDo(oldId, i.files[0]); };
    i.click();
  },
  ...
  async _replaceDo(oldId, file) {
    try {
      ...
      const meta = {
        ...
        size: file.size,
        type: this._type(file.name),
        uploader: Auth.getUser()?.full_name || '',
        uploaded_at: new Date().toISOString()
      };
      await API.replaceFile(this._openOrderId, oldId, meta);
      // 删除旧文件
      if (String(oldId).startsWith('cloud://')) {
        try { await TCB.deleteFile({ fileList: [oldId] }); } catch(e) {}
      }
      await this._load(this._openOrderId, '');
    } catch(e) {
      alert('替换失败: ' + (e.message || '未知错误'));
    }
  },
  ...
  async _del(orderId, fileId) {
    ...
    if (String(fileId).startsWith('cloud://')) {
      try { await TCB.deleteFile({ fileList: [fileId] }); } catch(e) {}
    }
  ...
  renderFileBtn(order, userRole) {
    ...
    if (n > 0) {
      return `<button class="file-btn file-has" style="background:#eaf3de;color:#3b6d11;font-weight:600;" onclick="FileOps.open('${order._id}','${App.escapeHtml(order.invoice_no)}','${userRole}')">📎 ${n}个文件</button>`;
    }
    if (userRole === 'sales') return '';
    return `<button class="file-btn" style="color:#bbb;border-color:#ddd;" onclick="FileOps.open('${order._id}','${App.escapeHtml(order.invoice_no)}','${userRole}')">📁 暂无</button>`;
  }
};
