    ...
    updateQty: function(el) {
      ...
      el.value = val;
        ...
        API.updateOrder(oid, {product_model: JSON.stringify(pm)}).then(function(){
          ...
          DB.collection('notifications').add({salesperson_name:o.salesperson_name,invoice_no:o.invoice_no,message:o.invoice_no+' '+(pm[idx].model||'型号'+(idx+1))+'到'+val+'/'+(pm[idx].qty||0)+'件',order_id:oid,read:false,created_at:new Date().toISOString()}).catch(function(){});
        }).catch(function(){});
      }).catch(function(){});
    },
    fillAll: function(el) {
      ...
      DB.collection(COL.ORDERS).doc(oid).get().then(function(r) {
        ...
        API.updateOrder(oid, {product_model: JSON.stringify(pm)}).then(function(){
          var cell = el.closest('[data-pm]');
          if (cell) { cell.setAttribute('data-pm', JSON.stringify(pm)); enhanceProductCells(); }
          DB.collection('notifications').add({salesperson_name:o.salesperson_name,invoice_no:o.invoice_no,message:o.invoice_no+' '+pm.length+'款产品全部到齐',order_id:oid,read:false,created_at:new Date().toISOString()}).catch(function(){});
        }).catch(function(){});
      }).catch(function(){});
    },
    completeOrder: function(oid) {
      ...
      DB.collection(COL.ORDERS).doc(oid).get().then(function(r){
        ...
        API.updateOrder(oid, {order_status: '已完结'}).then(function(r2) {
          ...
          DB.collection('notifications').add({salesperson_name:o.salesperson_name,invoice_no:o.invoice_no,message:o.invoice_no+' 已完结',order_id:oid,read:false,created_at:new Date().toISOString()}).catch(function(){});
